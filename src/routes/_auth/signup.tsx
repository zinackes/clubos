...
function RouteComponent() {
    ...
    const register = z.object({
        ...
        confirmPassword: z.string("Le mot de passe doit être une chaîne de caractère").min(8, "Le mot de passe doit comprendre 8 caractères minimum")
    ...
    const form = useForm({
        ...
        onSubmit: async (values) => {
            ...
            console.log(data);
            if(error){
                setSignUpError(getFrenchTranslation(error.code ?? ""))
                return;
            }
  ...
  return (
    <>
        <div className={"w-full flex-1 grid grid-cols-1 md:grid-cols-2 grid-rows-1 font-base"}>
            <div className={"flex flex-col  p-8 md:p-12 lg:p-16 justify-center relative z-10"}>
                <div className={"mb-10"}>
                    <h1 className={"font-title text-3xl md:text-4xl font-bold text-slate-900 mb-3 tracking-light"}>Commencez votre journée</h1>
                    <p className={"text-slate-500 text-md font-base"}>Rejoignez des milliers de clubs qui gèrent leurs activités sportives</p>
                </div>
                <form onSubmit={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                ...
                <p className={"mt-8 text-center text-sm text-slate-500"}>
                     Déja un compte ?
                    {" "}
                    <a href={"/login"} className={"font-semibold text-indigo-600 hover:text-indigo-500 transition-colors cursor-pointer"}>Connectez-vous</a>
                </p>
            </div>

            <div className={"hidden md:flex bg-indigo-900 relative overflow-hidden"}>
